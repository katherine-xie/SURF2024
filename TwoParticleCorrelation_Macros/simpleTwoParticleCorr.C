...
TH2F createEtaPhiDist(std::vector<Int_t> multiplicityVector) {
    ...
    TH2F etaPhiDist("etaPhiDist", "(#eta, #phi) Distribution for all Particles", 50, -etaCutOff, etaCutOff, 50, -TMath::Pi(), TMath::Pi());
...
TH2F createBackgroundDist(std::vector<Int_t> multiplicityVector, Int_t numMixFactor, TH2F hSignal, TH2F etaPhiDist) {
    ...
    //Int_t numPseudo = 12722; // epem (163 GeV)
    //Int_t numPseudo = 13474; // epem (240 GeV)
    //Int_t numPseudo = 34325; // mupmum (3 TeV)
    Int_t numPseudo = 54021; // mupmum (10 TeV)
...
void simpleTwoParticleCorr() {
    ...
    TCanvas *cCorrected = new TCanvas("cCorrected", "Canvas for the Corrected Signal Distribution", 5000, 5000);
    ...
    correctedHist.GetXaxis()->SetTitleSize(0.06);

    correctedHist.GetYaxis()->SetTitleOffset(1);
    correctedHist.GetYaxis()->SetTitleSize(0.06);
