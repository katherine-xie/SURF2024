        ...
        for(int wtrk = 1; wtrk < trackbin+1; wtrk++){
            ...
            for(int wppt = 1; wppt < ptbin+1; wppt++){ 
                ...
                for(int wpPU = 1; wpPU < PUbin+1; wpPU++){
                    ...
                    long int XENT =  ((1+floor(sqrt(1+(4*2*backMult*NENT))))/2) ; // Calculating the number of pseudoparticles needed
                    //float A_ETA[XENT] = {0};
                    //float A_PHI[XENT] = {0};
                    ...
                    for(int x = 0; x<XENT; x++){
                        ...
                        hEPDrawCor[wtrk-1][wppt-1][wpPU-1]->GetRandom2(WEta1_Cor, WPhi1_Cor); // Selecting the pseudoparticles
