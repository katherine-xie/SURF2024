...
void EventDisplayHorizontal_White(int eventIndx = 1234, 
		              float ptCut = 0, 
		              bool doWTA = false,
		              bool verbose = 0) {
  ...
  // Creating the canvas for the panels with 4 different views
  TCanvas *c = new TCanvas("c", "Event Display", 2000, 1000);  
  c->SetFillColor(0);
  c->Divide(4, 1, 0, 0, 0);
  ...
  // Canvas for the interactive viewer (main viewer)
  ...
  // std::cout << "Number of jets " << ": " << pPt->size() << std::endl;

  // ***** JET LOOP *****
  for (int j = 0; j < pPt->size(); j++) {

    // std::cout << "Size of jet " << j << ": " << pPt->at(j).size() << std::endl;
    ...
    for(int i = 0; i < (*pPt)[j].size(); i++) {
      ...
      formatHelixWhiteCanvas(helix[nHelix], pz, pt, trackColor, doWTA);
